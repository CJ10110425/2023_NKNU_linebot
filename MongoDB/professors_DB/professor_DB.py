...
def find_data_config(db_name: Collection) -> list:
    ...
    """
    valid_config = validation_rules[str(
        db_name.name)]["$jsonSchema"]["properties"]
...
def create_database(db_name: str) -> Collection:
    ...
    if db_name not in collection_list:
        professor_db = nknu_linebot_db.create_collection(
            db_name, validator=validation_rules)
    else:
...
def update_data(col_name: Collection,
    ...
    for data in update_config:
        if data not in data_config:
            raise ValueError(
                f"The update data does not match validation schema")

    result = col_name.update_one({"user_id": user_id}, {"$set": update_data})
    returndata = {"modified_info": result.modified_count,
                  "changed_info": data_config}
...
def find_professor_by_userid(user_id: str) -> dict:
...
if __name__ == "__main__":
    ...
    validation_path = Path("MongoDB/professors_DB/data_validation.json")
    ...
    print(find_data("professors_DB", 1))
